...
<a id="init"></a>
## Инициализация
Для работы Timer3 LGT8F328P необходимо обязательно выполнить команду `init_timer_pin(pin)` для `D1` и/или `D2`, использовать конструкцию `pinMode(pin, OUTPUT)` для них не надо. Для других пинов можно использовать любой из вариантов.

<a id="usage"></a>
## Использование
```cpp
// ============== Функции для расширенной генерации ШИМ сигнала ==============
// Данные функции убирают один ШИМ выход у 8-ми битных таймеров, оставляя нам ШИМ пины D3, D5, D9 и D10 на ATmega328 плюс D1 и D2 для LGT8F328P
void PWM_frequency(byte pin, long freq, modes correct);
/*  PWM_freqency(пин, частота, режим) - запустить ШИМ с выбранной частотой
 - Пины: D3 (таймер 2), D5 (таймер 0 - сломает millis/delay), D9 и D10 (таймер 1), D1 и D2 (таймер 3 LGT8F328P)
...
/*  PWM_resolution(пин, разрядность, режим) - запустить ШИМ с выбранной разрядностью
 - Пины: D3 (таймер 2), D5 (таймер 0 - сломает millis/delay), D9 и D10 (таймер 1), D1 и D2 (таймер 3 LGT8F328P)
 - Режим: 0 (FAST_PWM), 1 (CORRECT_PWM)
 - Разрешение: D3 (4-8 бит), D5 (4-8 бит), D9 и D10 (4-16 бит), D1 и D2 (4-16 бит)
...
/*  PWM_set(пин, заполнение) - изменить заполнение на выбранном пине
 - Пин: D3, D5, D6, D9, D10, D11; плюс D1 и D2 для LGT8F328P
...
|PWM    | 9, 10, 1,2|1 МГц  |516 кГц|254 кГц|126 кГц|63 кГц|31.2 кГц|15.6 кГц|7.8 кГц|3.9 кГц|1.9 кГц|980 Гц|488 Гц|244 Гц|
...
|PWM    | 9, 10, 1,2|571 кГц|266 кГц|129 кГц|63 кГц |32 кГц|15.7 кГц|7.8 кГц |3.9 кГц|1.9 кГц|976 Гц |488 Гц|244 Гц|122 Гц|
|_______|___________|_______|_______|_______|_______|______|________|________|_______|_______|_______|______|______|______|
*/
...
void PWM_TMR3_8BIT();  // Установить таймер 3 (ШИМ на D1 и D2) в режим 8 бит. См. таблицу частот
void PWM_TMR3_10BIT();  // Установить таймер 3 (ШИМ на D1 и D2) в режим 10 бит. См. таблицу частот
/*
...
|       | Timer1 (пины 9 и 10) 8 bit| Timer 3 (пины 1 и 2) 10 bit   |                           |
|       | Timer3 (пины 1 и 2) 8 bit |                               |                           |
...
void setup() {
  ...
  // пины 1 и 2 работают на одной частоте, запустить на разных не получится
}
